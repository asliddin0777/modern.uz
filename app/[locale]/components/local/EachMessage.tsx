...
const EachMessage = ({ userInfo, m, updateMessageViewStatus }: IProps) => {
    ...
    useEffect(() => {
        if (m.sender != userInfo.userId && m.viewed === false) { socket.emit("messageViewed", m); }
    }, []);
    return (
